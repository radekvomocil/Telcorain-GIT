...
from procedures.links_to_segments import linear_repeat, intersections
...
class Calculation(QRunnable):
    ...
    def run(self, ):
        ...
        try:
            ...
            linear_repeat(calc_data, isecDic, self.segment_size)
            intersections(calc_data, isecDic)
            ...
            rain_values = []
            for cml in range(0, calc_data.cml_id.size):
                for segment in range(0, calc_data.isel(cml_id=cml).segments.size):
                    idcko = calc_data.isel(cml_id=cml).cml_reference[segment].values
                    if bool(np.isnan(idcko)):
                        continue
                    int_idcko = int(idcko)
                    segment_value = float(rain_values_1h.sel(cml_id=int_idcko).mean(dim='channel_id').sum(dim='time'))
                    rain_values.append(segment_value)

            rain_grid = interpolator(x=longs_1dim,
                                     y=lats_1dim,
                                     z=np.asarray(rain_values),
                    ...
                    for cml in range(0, calc_data.cml_id.size):
                        for segment in range(0, calc_data.isel(cml_id=cml).segments.size):
                            ...
                            if bool(np.isnan(idcko)):
                                continue
                            int_idcko = int(idcko)
                            segment_value = float(rain_values_steps.sel(cml_id=int_idcko).mean(dim='channel_id').isel(time=x).values)
                            rain_values.append(segment_value)
